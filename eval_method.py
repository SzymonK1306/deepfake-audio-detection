...
from model_deepfake_method import DeepFakeMethodClassifier
...
class AudioFileDataset(Dataset):
    def __init__(self, root_dir, target_length=16000, n_mels=64):
        ...
        self.impulse_response, sr = torchaudio.load('TheSlot.wav')
        self.impulse_response = torch.mean(self.impulse_response, dim=0, keepdim=True)
                ...
                for root, _, files in os.walk(class_dir):
                    for file_name in files:
                        if file_name.endswith('.flac'):
                            ...
                            if 'Eleven' in class_dir:
                                self.labels.append(0)
                            elif 'Open' in class_dir:
                                self.labels.append(1)
                            elif 'BARK' in class_dir:
                                self.labels.append(2)
                            elif 'Toucan' in class_dir:
    ...
    def __getitem__(self, idx):
        ...
        # waveform = self.add_gaussian_noise(waveform, 50) # 65
        waveform = self.add_white_noise(waveform, 20)
        # waveform = self.add_pink_noise(waveform, 20)
        # waveform = self.convolve_with_impulse_response(waveform, self.impulse_response)
        ...
        # Add bubble noise to the waveform
        # bubble_noise = self.generate_bubble_noise(waveform.size(-1), waveform.device)
        # waveform = waveform + bubble_noise
        # waveform = self.add_gaussian_noise(waveform, 2)
    ...
    def generate_bubble_noise(self, length, device, amplitude=0.02, frequency=1.0):
        """
        Generate bubble noise for a given waveform length.
        Args:
            length (int): Number of samples in the waveform.
            device (torch.device): Device to create the tensor on.
            amplitude (float): Scaling factor for noise amplitude.
            frequency (float): Frequency of the bubble noise (lower values for slower variation).
        Returns:
            torch.Tensor: Bubble noise tensor of the same length as the waveform.
        """
        time = torch.linspace(0, length / self.target_sr, steps=length, device=device)
        noise = amplitude * torch.sin(2 * torch.pi * frequency * time)  # Bubble-like sine wave
        noise += amplitude * torch.randn(length, device=device) * 0.003  # Add some randomness
        return noise.unsqueeze(0)  # Match the channel dimension of the waveform

    def add_gaussian_noise(self, waveform, snr=20):
        """
        Adds Gaussian noise to the waveform while maintaining a target SNR.
        Args:
            waveform (torch.Tensor): Input waveform of shape (1, samples).
            snr (float): Signal-to-Noise Ratio in decibels (default 20 dB).
        Returns:
            torch.Tensor: Waveform with added Gaussian noise.
        """
        signal_power = torch.mean(waveform ** 2)
        noise_power = signal_power / (10 ** (snr / 10))  # Calculate noise power based on SNR
        gaussian_noise = torch.randn_like(waveform) * torch.sqrt(noise_power)
        return waveform + gaussian_noise
    ...
    def convolve_with_impulse_response(self, waveform, impulse_response):
        """
        Convolve the waveform with an impulse response to simulate room effects.
        Args:
            waveform (torch.Tensor): Input waveform of shape (1, samples).
            impulse_response (torch.Tensor): Impulse response of shape (1, ir_samples).
        Returns:
            torch.Tensor: Convolved waveform.
        """
        convolved = torch.nn.functional.conv1d(
            waveform.unsqueeze(0), impulse_response.unsqueeze(0), padding=impulse_response.size(-1) // 2
        )
...
def check_class_distribution(loader):
    ...
    for _, labels in loader:
        ...
        counts[2] += (labels == 0).sum().item()
        counts[3] += (labels == 1).sum().item()
    return counts
...
model = DeepFakeMethodClassifier('resnet18').to(device)
model.load_state_dict(torch.load('resnet18_method_16k.pth'))
...
sns.heatmap(cm, annot=True, fmt="d", cmap="Blues", xticklabels=['ElevenLabs', 'OpenVoice', 'Bark', 'Toucan'], yticklabels=['ElevenLabs', 'OpenVoice', 'Bark', 'Toucan'])
